...
const courseController = {
  async createClass(req, res) {
    ...
    try {
      const newClass = await courseService.createCourse({
        name,
      });
      return res
        .status(201)
  ...
  async getClassById(req, res) {
    ...
    try {
      const getClass = await courseService.getClassById(id);
      if (!getClass) {
        return res
          .status(404)
  ...
  async updateClass(req, res) {
    ...
    try {
      const updatedClass = await courseService.updateClass(id, {
        name,
        grade,
        age,
      });
  ...
  async deleteClass(req, res) {
    ...
    try {
      const deleteClass = await courseService.deleteClass(id);
      if (!deleteClass) {
        return res
          .status(404)
          .json({ status: 404, message: "Class not found" });
